...
/**
 * Returns the dependency scope which has flags pecular to the both provided scopes
 */
fun getMaximalScope(scope1: DependencyScope, scope2: DependencyScope?): DependencyScope {
    if (scope2 == null) {
        return scope1
    }
...
fun addModuleDependencyIfNeeded(
    ...
    val requiredScope = getMaximalScope(if (testScope) DependencyScope.TEST else DependencyScope.COMPILE, existingEntry?.scope)
